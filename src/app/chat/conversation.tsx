...
import Notification, {NotificationProps} from "@/app/_components/Notification";
...
export default function Conversation() {
  ...
  const [serviceSummary, setServiceSummary] = useState<RecommendFunctionsSummary | null>(null);
  ...
  trpc.conversation.recommendServices.useSubscription({roomId, userInput}, {
  ...
  return (
    <>
      <Notification open={noti.open} message={noti.message} title={noti.title} type={noti.type}/>
        ...
        <header className="flex flex-row h-16 w-full bg-gray-100 dark:bg-gray-800 px-8">
          ...
          <div className="flex flex-row items-center ml-auto gap-2 dark:text-white">
            <button
              ...
              Create Room
              <PlusIcon className="h-5 w-5 rounded-full dark:text-black dark:bg-white" aria-hidden="true"/>
            </button>
          </div>
        </header>
        <main className="flex flex-row h-full md:container gap-5 p-2">
          <div
            className="flex flex-col justify-center w-1/2 gap-2.5 border px-4 py-2 rounded-lg border-gray-100 shadow-md h-full overflow-y-auto">
            ...
            />
            {
              room.isSuccess && (room.data?.history ?? []).map(({userInput, botAnswer}, i) => (
                <>
                  <ChatBubble key={`chat-${i}-user`}
                              name="user"
                              message={userInput}
                              self={true}
                              markdown={false}
                  />
                  {botAnswer && <ChatBubble key={`chat-${i}-assistant`}
                                            name="assistant"
                                            message={botAnswer ?? ""}
                                            self={false}
                                            markdown={true}
            ...
            <form className="flex flex-col justify-end h-full" onSubmit={(e) => {
              ...
              <div className="relative mt-2 rounded-md shadow-sm border-none">
                <input type="text"
                       ...
                       ref={questionRef}
                />
                <button className="absolute inset-y-0 right-0 flex items-center pr-3 hover:cursor-pointer"
                        type='submit'>
              ...
              serviceSummary !== null && <>
                ...
                <div className="flex flex-row gap-2.5">
                  ...
                  {serviceSummary.functions.map((f, i) => (
                    <Badge key={`f-${i}`} className="max-w-sm">{f}</Badge>
                  ))}
                </span>
                </div>
              </>
